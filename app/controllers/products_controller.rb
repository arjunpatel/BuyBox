class ProductsController < ApplicationController
  ...
  def buybox_genie
    ...
    render :json => {:price => "$ #{(total/parsed_json["results"].size).ceil}", :brand => parsed_json["results"][0]["make"]}
  end

  def index
    @search = Product.search(params[:search])
    @products = @search.where(:active => true).where("quantity > 0").order("popularity_index DESC") # or @search.relation to lazy load in view
  end
